...
export function FormulaTree({ node }: { node: FormulaStructureNode }) {
  return (
    <div className="grid gap-3">
      <div className="rounded-lg border border-indigo-100 bg-indigo-50 p-4 dark:border-indigo-500/30 dark:bg-indigo-500/15">
      ...
      {!!node.children?.length && (
        <div className="grid gap-3 border-l-2 border-slate-200 pl-4">
          {node.children.map((child) => (
            <div key={child.id} className="relative rounded-lg border border-lens-line bg-white p-3 dark:bg-slate-950">
